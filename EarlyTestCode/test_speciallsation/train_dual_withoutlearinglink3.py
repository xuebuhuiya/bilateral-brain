...
class ResNet9Features(nn.Module):
    def __init__(self, original_model):
        ...
        # 添加 1x1 卷积层以调整通道数，并使用适当的池化层调整空间尺寸
        self.adjust_channels1 = nn.Sequential(
            nn.Conv2d(512, 128, kernel_size=1, bias=False),
            nn.AdaptiveAvgPool2d((16, 16))
        )
        self.adjust_channels2 = nn.Sequential(
            nn.Conv2d(512, 256, kernel_size=1, bias=False),
            nn.AdaptiveAvgPool2d((8, 8))
        )
        self.adjust_channels_res2 = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=1, bias=False),  # 假设交换的特征来自256通道
            nn.AdaptiveAvgPool2d((4, 4))  # 确保输出尺寸是4x4
        )
        
        # 新增卷积层调整拼接后的通道数
        self.adjust_concat1 = nn.Conv2d(256, 128, kernel_size=1, bias=False)  # 假设你拼接后的通道是2倍
    ...
    def forward(self, x, other_features=None):
        ...
        # 检查 other_features 并确保其含有足够的元素
        if other_features is not None and len(other_features) > 2:
            other_res1 = self.adjust_channels1(other_features[2])  # 确保这里不会出现索引错误
            res1 = torch.cat((res1, other_res1), dim=1)
            res1 = self.adjust_concat1(res1)
        elif other_features is not None:
            print("Warning: other_features does not have enough elements for index 2.")
        
        layer2 = self.layer2(res1)
        if other_features is not None and len(other_features) > 2:
            other_layer2 = self.adjust_channels2(other_features[2])  # 使用相同的res2特征
        ...
        if other_features is not None and len(other_features) > 1:
            other_res2 = self.adjust_channels_res2(other_features[1])  # 使用layer2特征
            res2 = torch.cat((res2, other_res2), dim=1)
            res2 = self.adjust_concat_res2(res2)
        elif other_features is not None:
            print("Warning: other_features does not have enough elements for index 1.")

        # 返回最终输出res2和包含所有中间层输出的列表
        return res2, [res1, layer2, res2]
...
class CombinedModel(nn.Module):
    ...
    def forward(self, x_coarse, x_fine):
        ...
        coarse_final, coarse_intermediates = self.coarse_features_model(x_coarse, other_features=fine_intermediates)
        fine_final, fine_intermediates = self.fine_features_model(x_fine, other_features=coarse_intermediates)
