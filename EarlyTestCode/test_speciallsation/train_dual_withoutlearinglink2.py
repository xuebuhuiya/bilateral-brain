...
class ResNet9Features(nn.Module):
    ...
    #     print(f'Original res1 shape: {res1.shape}')
    #     if other_features is not None:
    #         other_res1 = self.adjust_channels1(other_features[0])
    #         print(f'Other res1 shape after adjust_channels1: {other_res1.shape}')
    #         res1 = torch.cat((res1, other_res1), dim=1)
    #         print(f'Concatenated res1 shape: {res1.shape}')
        
    #     layer2 = self.layer2(res1)
    #     print(f'Original layer2 shape: {layer2.shape}')
    #     if other_features is not None:
    #         other_layer2 = self.adjust_channels2(other_features[1])
    #         print(f'Other layer2 shape after adjust_channels2: {other_layer2.shape}')
    #         layer2 = torch.cat((layer2, other_layer2), dim=1)
    #         print(f'Concatenated layer2 shape: {layer2.shape}')
        
    #     layer3 = self.layer3(layer2)
    #     res2 = layer3 + self.res2(layer3)
    #     print(f'Original res2 shape: {res2.shape}')
    #     if other_features is not None:
    #         other_res2 = self.adjust_channels_res2(other_features[2])
    #         print(f'Other res2 shape after adjust_channels_res2: {other_res2.shape}')
    #         res2 = torch.cat((res2, other_res2), dim=1)
    #         print(f'Concatenated res2 shape: {res2.shape}')
...
class CombinedModel(nn.Module):
    ...
    def forward(self, x_coarse, x_fine):
        ...
        fine_features, fine_intermediate = self.fine_features_model(x_fine)
        
        
        # coarse_features, _ = self.coarse_features_model(x_coarse, other_features=fine_intermediate)
        fine_features, _ = self.fine_features_model(x_fine, other_features=coarse_intermediate)
        coarse_features, _ = self.coarse_features_model(x_coarse, other_features=fine_intermediate)
        # coarse_features, _ = self.coarse_features_model(x_coarse, other_features=fine_intermediate)
