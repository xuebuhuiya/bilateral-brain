...
def plot_learning_curves(training_losses, training_accuracies, validation_accuracies_coarse, validation_accuracies_fine):
    ...
    plt.subplot(1, 2, 2)
    plt.plot(epochs, training_accuracies, label='Training Accuracy', color='red')
    plt.plot(epochs, validation_accuracies_coarse, label='Validation Accuracy Coarse', color='blue')
    plt.plot(epochs, validation_accuracies_fine, label='Validation Accuracy Fine', color='green')
    plt.title('Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    ...
    save_path = f"{filename}_Learning_Curve.png"
...
# 数据预处理和加载
    ...
    for epoch in range(num_epochs):
        ...
        for (coarse_data, fine_data) in tqdm(zip_longest(train_coarse_loader, train_fine_loader), total=len(train_coarse_loader), desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            correct_train += (predicted_coarse == coarse_labels).sum().item()
            correct_train += (predicted_fine == fine_labels).sum().item()
            total_train += coarse_labels.size(0) + fine_labels.size(0)


        epoch_loss = running_loss / len(train_coarse_loader)
    ...
    # 评估模型
        ...
        with torch.no_grad():
            ...
            accuracy_coarse = 100 * correct_coarse / total_coarse
            accuracy_fine = 100 * correct_fine / total_fine
            validation_accuracies_coarse.append(accuracy_coarse)
            validation_accuracies_fine.append(accuracy_fine)
    ...
    plot_learning_curves(training_losses, training_accuracies, validation_accuracies_coarse, validation_accuracies_fine)
